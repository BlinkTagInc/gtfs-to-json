...
const buildAgencyJSON = async (agencyKey, config, outputStats) => {
  ...
  const routesJSON = uniqBy(routes, 'route_short_name');

  for (const route of routesJSON) {
    const stops = db
      .prepare(
        'SELECT DISTINCT stops.stop_id, stops.stop_name, stops.stop_lat, stops.stop_lon from stops INNER JOIN stop_times ON stops.stop_id = stop_times.stop_id INNER JOIN trips on trips.trip_id = stop_times.trip_id WHERE trips.route_id = ? ORDER BY stops.stop_name',
      )
      .all(route.route_id);
